...
export default function SearchInput() {
	...
	return (
		<div>
           ...
           <HiSearch onClick={()=>setClick(!click)} className='absolute lg:top-[13px] md:top-[9px] top-[3px] lg:ms-[7rem] md:ms-3 hover:bg-[#3c404314] cursor-pointer w-12 h-12 lg:w-10 lg:h-10 p-3 hover:rounded-full duration-100'/><input value={searched} onChange={handleChange} id="search_input" type="text" placeholder='Search' className={`lg:w-[600px] md:w-[400px] w-[12rem] p-3 bg-[#3c404314] outline-none rounded-lg md:px-16 pe-1 cursor-pointer absolute top-[5px] left-0 md:static bg-white ms-2 lg:ms-auto shadow shadow-gray-950 ${click ? "hidden" : "block"} md:bg-transparent md:block`}/> 
		   {!!searched.length && (
           <AiOutlineClose onClick={handleReset} className={`absolute md:top-[20px] lg:top-[23px] lg:left-[785px] md:left-[595px]`}/>
			)}
           </div>
		</div>
